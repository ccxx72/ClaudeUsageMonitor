...
[VERSIONE RESPONSIVE - Finestra ridimensionabile con font dinamici]
[RESPONSIVE VERSION - Resizable window with dynamic fonts]

IT: Mostra l'uso del piano Claude.ai (sessione 5h + settimanale) in una
    finestra ridimensionabile, con elementi che si adattano dinamicamente
    alla dimensione dello schermo. Icona nella system tray che riporta solo %.
EN: Shows Claude.ai plan usage (5h session + weekly window) in a resizable
    window, with elements that dynamically adapt to the screen size. 
    System-tray icon displaying just the percentage.
...
Avvio / Run:  python claude_usage_monitor_responsive.py
=====================================================================
"""
...
# ---- Dimensioni di default (saranno scalate) / Default sizes (will be scaled) ----
DEFAULT_WIDTH  = 380
DEFAULT_HEIGHT = 270
MIN_WIDTH      = 150
MIN_HEIGHT     = 100
...
# ===================== UI =====================
class App:
    """IT: finestra principale Tkinter + gestione tray (RESPONSIVE).
       EN: main Tkinter window + tray management (RESPONSIVE)."""
    ...
    def __init__(self, root):
        ...
        # IT: finestra ridimensionabile con dimensioni min
        # EN: resizable window with minimum dimensions
        root.resizable(True, True)
        root.minsize(MIN_WIDTH, MIN_HEIGHT)
        root.geometry(f"{DEFAULT_WIDTH}x{DEFAULT_HEIGHT}")

        # IT: area di disegno (scalabile)
        # EN: drawing area (scalable)
        self.canvas = tk.Canvas(root, bg=COL_BG, highlightthickness=0)
        self.canvas.pack(side="top", fill="both", expand=True)
        
        # IT: bind per il ridimensionamento
        # EN: bind for resizing
        ...
        # IT: ultimo aggiornamento della finestra (per evitare ridisegni troppo frequenti)
        # EN: last window update (to avoid too-frequent redraws)
    ...
    # ---------- gestione ridimensionamento / resize handling ----------
    def _on_canvas_resize(self, event):
        """IT: schedula un ridisegno quando il canvas viene ridimensionato.
           EN: schedule a redraw when the canvas is resized."""
        if not self._scheduled_draw:
            self._scheduled_draw = True
            self.root.after(100, self._do_scheduled_draw)

    def _do_scheduled_draw(self):
        """IT: esegue il ridisegno dopo una pausa (evita ridisegni troppo frequenti).
           EN: perform redraw after a delay (avoid too-frequent redraws)."""
        self._scheduled_draw = False
        self.draw()

    def _get_scale_factor(self):
        """IT: calcola il fattore di scala basato sulla dimensione del canvas.
           EN: calculate scale factor based on canvas size."""
        w = self.canvas.winfo_width()
        h = self.canvas.winfo_height()
        if w < 1 or h < 1:
            w, h = DEFAULT_WIDTH, DEFAULT_HEIGHT
        
        scale_w = w / DEFAULT_WIDTH
        scale_h = h / DEFAULT_HEIGHT
        # IT: usa il minore per mantenere le proporzioni / EN: use the smaller for aspect ratio
        return min(scale_w, scale_h)

    def _get_scaled_font_size(self, base_size):
        """IT: calcola la dimensione del font scalato.
           EN: calculate the scaled font size."""
        scale = self._get_scale_factor()
        return max(8, int(base_size * scale))

    def _update_button_fonts(self):
        """IT: aggiorna i font dei pulsanti in base alla scala.
           EN: update button fonts based on scale."""
        size = self._get_scaled_font_size(11)
        btn_font = tkfont.Font(family="Helvetica", size=size, weight="bold")
        try:
            self.btn_refresh.config(font=btn_font, padx=int(14 * self._get_scale_factor()), 
                                   pady=int(6 * self._get_scale_factor()))
            self.btn_key.config(font=btn_font, padx=int(14 * self._get_scale_factor()), 
                               pady=int(6 * self._get_scale_factor()))
        except Exception:
            pass
    ...
    def draw(self):
        """IT: ridisegna l'intero contenuto della finestra (responsivo).
           EN: redraw the entire window content (responsive)."""
        import time
        now = time.time()
        if now - self._last_draw_time < 0.05:
            # IT: evita ridisegni troppo frequenti / EN: avoid too-frequent redraws
            return
        ...
        # IT: determina se siamo in modalità "compatta" (spazio limitato)
        # EN: determine if we're in "compact" mode (limited space)
        is_compact = w < 220 or h < 130

        # IT: calcola i font scalati / EN: calculate scaled fonts
        f_title = tkfont.Font(family="Helvetica", size=self._get_scaled_font_size(22), weight="bold")
        f_small = tkfont.Font(family="Helvetica", size=self._get_scaled_font_size(11), weight="bold")
        f_big = tkfont.Font(family="Helvetica", size=self._get_scaled_font_size(58), weight="bold")
        f_pct = tkfont.Font(family="Helvetica", size=self._get_scaled_font_size(22), weight="bold")
        f_cd = tkfont.Font(family="Helvetica", size=self._get_scaled_font_size(18), weight="bold")
        f_status = tkfont.Font(family="Helvetica", size=self._get_scaled_font_size(9))

        # IT: aggiorna font pulsanti / EN: update button fonts
        self._update_button_fonts()

        # IT: spaziature scalate / EN: scaled padding
        pad_h = int(14 * scale)
        pad_v = int(10 * scale)

        if is_compact:
            # ============== MODALITA' COMPATTA ==============
            # IT: mostra solo essenziale: percentuale e countdown
            # EN: show only essential: percentage and countdown
            
            # IT: intestazione minimalista / EN: minimal header
            c.create_text(pad_h, pad_v, anchor="nw", text="CLAUDE",
                          fill=COL_ACCENT, font=f_title)
            ...
            if not S.data_ok:
                ...
                c.create_text(w // 2, h // 2, text=msg,
                              fill=COL_DIM, font=f_small)
                self.draw_status(c, f_status, w, h, scale)
                self.update_tray()
                return

            # IT: percentuale grande al centro / EN: large percentage in the center
            ps = (str(int(round(S.session_pct)))
                  if S.session_pct is not None else "--")
            
            # IT: dimensione font adattata per lo spazio disponibile / EN: font size adapted to available space
            pct_font_size = min(self._get_scaled_font_size(58), int(h * 0.4))
            f_big_compact = tkfont.Font(family="Helvetica", size=max(14, pct_font_size), weight="bold")
            
            c.create_text(pad_h, h // 2 - int(15 * scale), anchor="nw", text=ps,
                          fill=COL_TEXT, font=f_big_compact)
            
            w_ps = f_big_compact.measure(ps)
            c.create_text(pad_h + w_ps + int(4 * scale), h // 2 - int(5 * scale), 
                          anchor="nw", text="%", fill=COL_TEXT, font=f_small)

            # IT: countdown compatto a destra / EN: compact countdown on the right
            countdown_text = fmt_countdown(S.session_reset)
            c.create_text(w - pad_h, h // 2 - int(8 * scale), anchor="ne",
                          text=countdown_text, fill=COL_ACCENT, font=f_cd)
            ...
            c.create_text(pad_h, pad_v, anchor="nw", text="CLAUDE",
                          fill=COL_ACCENT, font=f_title)
            c.create_text(w - pad_h, pad_v + 6, anchor="ne", text="uso piano",
                          fill=COL_DIM, font=f_small)

            # IT: nessun dato valido -> messaggio centrale / EN: no valid data -> centered message
            if not S.data_ok:
                msg = S.err_msg or "In attesa dati..."
                c.create_text(w // 2, h // 2 - int(10 * scale), text=msg,
                              fill=COL_DIM, font=f_pct)
                self.draw_status(c, f_status, w, h, scale)
                self.update_tray()
                return

            # ---- SESSIONE (5h) / 5h SESSION ----
            y_session = int(56 * scale)
            c.create_text(pad_h, y_session, anchor="nw", text="SESSIONE (5h)",
                          fill=COL_DIM, font=f_small)
            
            y_session_val = int(70 * scale)
            ps = (str(int(round(S.session_pct)))
                  if S.session_pct is not None else "--")
            c.create_text(pad_h, y_session_val, anchor="nw", text=ps,
                          fill=COL_TEXT, font=f_big)
            
            w_ps = f_big.measure(ps)
            c.create_text(pad_h + w_ps + int(6 * scale), y_session_val + int(30 * scale), 
                          anchor="nw", text="%",
                          fill=COL_TEXT, font=f_pct)

            # IT: countdown al reset della sessione / EN: countdown to session reset
            c.create_text(w - pad_h, y_session, anchor="ne", text="ripristino tra",
                          fill=COL_ACCENT, font=f_small)
            c.create_text(w - pad_h, y_session + int(16 * scale), anchor="ne",
                          text=fmt_countdown(S.session_reset),
                          fill=COL_ACCENT, font=f_cd)

            # IT: barra di avanzamento sessione / EN: session progress bar
            y_bar_session = int(158 * scale)
            bar_h = int(14 * scale)
            x0, x1 = pad_h, w - pad_h
            c.create_rectangle(x0, y_bar_session, x1, y_bar_session + bar_h, outline=COL_DIM)
            if S.session_pct is not None and S.session_pct > 0:
                bw = int((x1 - x0 - 2) * min(S.session_pct, 100) / 100.0)
                if bw > 0:
                    c.create_rectangle(x0 + 1, y_bar_session + 1, x0 + 1 + bw, y_bar_session + bar_h - 1,
                                       fill=COL_ACCENT, outline="")

            # ---- SETTIMANALE / WEEKLY ----
            y_weekly = int(184 * scale)
            c.create_text(pad_h, y_weekly, anchor="nw", text="SETTIMANALE",
                          fill=COL_DIM, font=f_small)
            
            y_weekly_val = int(200 * scale)
            if S.weekly_pct is not None:
                ws = (f"{int(round(S.weekly_pct))}%   reset "
                      f"{fmt_countdown(S.weekly_reset)}")
            else:
                ws = "--"
            c.create_text(pad_h, y_weekly_val, anchor="nw", text=ws,
                          fill=COL_TEXT, font=f_pct)

            # IT: barra di avanzamento settimanale / EN: weekly progress bar
            y_bar_weekly = int(232 * scale)
            c.create_rectangle(x0, y_bar_weekly, x1, y_bar_weekly + bar_h, outline=COL_DIM)
            if S.weekly_pct is not None and S.weekly_pct > 0:
                bw = int((x1 - x0 - 2) * min(S.weekly_pct, 100) / 100.0)
                if bw > 0:
                    c.create_rectangle(x0 + 1, y_bar_weekly + 1, x0 + 1 + bw, y_bar_weekly + bar_h - 1,
                                       fill=COL_DIM, outline="")

        self.draw_status(c, f_status, w, h, scale)
        self.update_tray()  # IT: tiene allineata l'icona tray / EN: keeps the tray icon in sync

    def draw_status(self, c, f_status, w, h, scale):
        ...
        if S.fetching:
            ...
            txt = ""
        c.create_text(int(14 * scale), h - int(4 * scale), anchor="sw", text=txt,
                      fill=COL_DIM, font=f_status)
    ...
    def on_set_key(self):
        ...
        # IT: calcola font per il dialog / EN: calculate fonts for dialog
        scale = self._get_scale_factor()
        f_small = tkfont.Font(family="Helvetica", size=self._get_scaled_font_size(11), weight="bold")
        f_status = tkfont.Font(family="Helvetica", size=self._get_scaled_font_size(9))

        tk.Label(
            dlg, bg=COL_BG, fg=COL_DIM, font=f_small, justify="left",
        ...
        tk.Button(row, text="Salva", command=do_save, bg=COL_ACCENT,
                  fg=COL_BG, relief="flat", bd=0, font=f_small,
